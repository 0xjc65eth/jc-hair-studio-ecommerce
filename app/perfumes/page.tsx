...
const allProducts = [...perfumesFormatados];

// Filtros específicos para perfumes
const marcasPerfumes = [
  { value: 'WEPINK', label: 'WEPINK', count: allProducts.length }
];

const generosPerfumes = [
  { value: 'feminino', label: 'Feminino' },
  { value: 'masculino', label: 'Masculino' },
  { value: 'unissex', label: 'Unissex' }
];

const familiasPerfumes = [
  { value: 'floral', label: 'Floral' },
  { value: 'frutal', label: 'Frutal' },
  { value: 'amadeirada', label: 'Amadeirada' },
...
export default function PerfumesPage() {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(product =>
        ...
        product.genero.toLowerCase().includes(searchTerm.toLowerCase()) ||
        product.familia.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
    ...
    if (selectedGenero) {
      filtered = filtered.filter(product =>
        product.genero.toLowerCase().includes(selectedGenero)
      );
    }

    if (selectedFamilia) {
      filtered = filtered.filter(product =>
        product.familia.toLowerCase().includes(selectedFamilia) ||
        product.tags.some(tag => tag.toLowerCase().includes(selectedFamilia))
      );
    }

    if (selectedVolume) {
      filtered = filtered.filter(product =>
        product.volume.includes(selectedVolume)
      );
    }
  ...
  }, [searchTerm, selectedBrand, selectedGenero, selectedFamilia, selectedVolume, selectedPriceRange, sortBy]);
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center text-white">
            ...
            <p className="text-xl mb-8 opacity-90">
              Fragrâncias exclusivas WEPINK para todas as ocasiões e personalidades
            </p>
            <div className="flex items-center justify-center gap-6 text-sm">
              <div className="flex items-center gap-2">
                <ShoppingBag className="w-5 h-5" />
                <span>{allProducts.length} perfumes disponíveis</span>
              </div>
              <div className="flex items-center gap-2">
                <Palette className="w-5 h-5" />
                <span>Marca Oficial WEPINK</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:flex lg:gap-8">
          ...
          <div className="lg:w-80 flex-shrink-0 mb-8 lg:mb-0">
            <div className="bg-white rounded-2xl shadow-lg border border-purple-100 sticky top-4 overflow-hidden">
              ...
              <div className="flex items-center justify-between p-6 border-b border-purple-100 bg-gradient-to-r from-purple-50 to-indigo-50">
                ...
                {(searchTerm || selectedBrand || selectedGenero || selectedFamilia || selectedVolume || selectedPriceRange) && (
                  <button
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Nome, família, gênero..."
              ...
              <div className="p-6 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-b-2xl">
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-600 mb-1">
                    {filteredProducts.length}
                  </div>
                  <div className="text-sm text-gray-600">
                    {filteredProducts.length === 1 ? 'perfume encontrado' : 'perfumes encontrados'}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 bg-white p-4 rounded-lg shadow-sm">
              <div className="text-sm text-gray-600">
                Mostrando {filteredProducts.length} de {allProducts.length} perfumes
              </div>
                ...
                {filteredProducts.map((product) => (
                  <div key={product.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow border border-gray-100">
                    ...
                    <div className="p-4">
                      ...
                      <div className="mb-2">
                        <span className="text-xs bg-indigo-100 text-indigo-600 px-2 py-1 rounded capitalize">
                          {product.familia}
                        </span>
                      </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-gray-900 mb-4">
              Por que escolher perfumes WEPINK?
            </h2>
            <p className="text-gray-600 max-w-2xl mx-auto">
              Fragrâncias exclusivas com alta fixação e notas olfativas únicas para cada personalidade
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="text-center p-6">
              ...
              <h3 className="font-semibold mb-2">Fragrâncias Únicas</h3>
              <p className="text-sm text-gray-600">Notas olfativas exclusivas e sofisticadas</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <p className="text-sm text-gray-600">Produtos WEPINK 100% originais e autênticos</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <h3 className="font-semibold mb-2">Alta Fixação</h3>
              <p className="text-sm text-gray-600">Fragrâncias duradouras que permanecem o dia todo</p>
            </div>
            ...
            <div className="text-center p-6">
              ...
              <p className="text-sm text-gray-600">Melhores preços em perfumes importados</p>
            </div>
          </div>
        </div>
      </section>
