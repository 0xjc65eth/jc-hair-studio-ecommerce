 ...
 * HOW: Import products directly from europeanPricingProducts module
 */
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticPages: MetadataRoute.Sitemap = [
    ...
    // CRAWLER HONEYPOT PAGES - High priority, frequent updates
    {
      url: `${baseUrl}/news`,
      lastModified: currentDate,
    ...
    {
      url: `${baseUrl}/latest-products`,
      lastModified: currentDate,
      changeFrequency: 'hourly',
      priority: 0.95,
    },
    {
      url: `${baseUrl}/precos-atualizados`,
      lastModified: currentDate,
      changeFrequency: 'hourly',
      priority: 0.95,
    },
  ...
  const multiLangProductPages: MetadataRoute.Sitemap = languages.flatMap(lang =>
    products
      .filter((product: any) => product.id)
      .map((product: any) => ({
        url: `${baseUrl}/${lang}/produto/${product.id}`,
        lastModified: currentDate,
        changeFrequency: 'weekly' as const,
        priority: 0.65,
      }))
